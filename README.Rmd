...
The simple mode allows the tools to be executed through a single function in which the configuration file (with the option to use the default configuration file, **configurationsTemplate.ini** in the case that the value of the parameter is NULL), the option to edit the input configuration file, the files and/or the folders that contains the files to be preprocessed, the flow of selected *Pipes* and the mode in which that the types of *Instances* are created. 
