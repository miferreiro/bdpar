...
#'
#' @description \code{Bdp4aR} class provides the static variables required
...
Bdpa4R <- R6Class(
  ...
  public = list(
    ...
    initialize = function(configurationFilePath = NULL,
      ...
      Bdpa4R[["private_fields"]][["configurationFilePath"]] <- configurationFilePath
      Bdpa4R[["private_fields"]][["connections"]] <- Connections$new(configurationFilePath)
    ...
    proccess_files = function(filesPath,
      ...
      if (!"character" %in% class(filesPath)) {
        stop("[Bdpa4R][proccess_files][Error]
      ...
      cat("[Bdpa4R][proccess_files][Info] ", "Has been created: ", length(InstancesList)," instances.\n")
