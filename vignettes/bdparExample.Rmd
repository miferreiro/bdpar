...
<div style="text-aling:justify">
For this preprocessing we will use both the default preprocessing flow (SerialPipes) and the way to create the Instances (InstanceFactory). Regarding the configuration file as a preprocess files are in the directory tree of the package.

The configuration file used will have the following form:
...
[CSVPath]
outPutTeeCSVPipePath = example.csv
```
...
pipeline_execute( configurationFilePath = system.file(package = "bdpar", "examples/configurationsExample.ini"), 
                  filesPath = system.file(package = "bdpar", "testFiles"))
```
